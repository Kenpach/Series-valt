...
export default function ShareModal({ open, onClose }) {
  ...
  const [initialType, setInitialType] = useState('ALL');
  ...
  async function generate() {
    ...
    setLink(`${origin}/share/${token}${qs ? `?${qs}` : ''}`);
  ...
  return (
    <div style={modalOverlayStyle} onClick={onClose}>
      <div style={{ ...modalCardStyle, width:'min(560px, 100%)' }} onClick={(e) => e.stopPropagation()}>
        ...
        <label className="modalLabel">Filtro inicial (opcional)</label>
        <div className="row" style={{ alignItems:'center' }}>
          <select className="input" style={{ maxWidth: 180 }} value={initialType} onChange={(e) => { setInitialType(e.target.value); if (e.target.value !== 'MOVIE') setInitialCategory('ALL'); }}>
            <option value="ALL">Todos</option>
            <option value="ANIME" disabled={!shareAnime}>Anime</option>
            <option value="SERIES" disabled={!shareSeries}>Série</option>
            <option value="MOVIE" disabled={!shareMovie}>Filme</option>
          </select>
          <select className="input" style={{ maxWidth: 220 }} value={initialStatus} onChange={(e) => setInitialStatus(e.target.value)}>
            <option value="ALL">Todos status</option>
            <option value="planned">Quero ver</option>
        ...
        {shareMovie && initialType === 'MOVIE' ? (
          <div style={{ marginTop: 10 }}>
            <label className="modalLabel">Categoria (Filme)</label>
            <select className="input" value={initialCategory} onChange={(e) => setInitialCategory(e.target.value)}>
              {['ALL','Ação','Comédia','Drama','Terror','Ficção','Romance','Documentário','Animação','Outros'].map((c) => (
